...
sayHelloButton.addEventListener('mouseout',original)
...
*/
const testRepeat = document.querySelector('#repeat-text');
const repeatMyParam = () => {

    
    axios.get(`http://localhost:3000/repeat/Jonah`).then(response=>
    {
        console.log(response.data);
        testRepeat.textContent = response.data;
        testRepeat.style.display = "block";
    })

}

const buttonToRepeat= document.querySelector('#repeat-button');
buttonToRepeat.addEventListener('click',repeatMyParam);

// PROBLEM 7
/*
    Now that we have the response data, let's add it to our web page! 
    
    Inside the repeatMyParam function above, grab the element with the id of 'repeat-' and set its textContent property equal to the response data.
*/
...
*/
const btnToQuery = document.querySelector('#query-button');

function attachQuery(){
    axios.get(`http://localhost:3000/query-test/?winner=he&?winner= to`).then(response => {
        console.log(response.data)
        testRepeat.textContent = response.data;
        testRepeat.style.display = "block";
    })
...
btnToQuery.addEventListener('click',attachQuery);
...
function clearHTML(){
    const clearPara = document.querySelector('p');
    clearPara.innerHTML = "";
}
...
function createFood(event){
    ...
    axios.post(`http://localhost:3000/food`, body).then((response)=>{
        ...
        for(let i = 0; i < response.data.length; i++)
        {
            const foodToHTML = document.createElement('p');
            foodToHTML.textContent = response.data[i];
            newDiv.append(foodToHTML);
            foodToHTML.style.display = 'block';
        }
    })
...
formEvent.addEventListener('submit',createFood)